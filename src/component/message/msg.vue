<template>
  <div class="w-all">
    <div @mouseenter="startTime(true)" @mouseleave="clearTimer" v-if="['success','error','warning'].includes(type)" :class="{'messageFadeInDown':isShow,'messageFadeOutUp':!isShow,'abst':type=='phone','at20':type!='phone'}" class="fixed ai-c zi-9999 abs absl">
      <div :class="{'trans-messge':type=='phone'}" class="ra-4 flex-line pl10 pr10 lh-22 pt8 pb3 sha-4 bc-fff _msg-box">
        <span>
          <svg v-if="type!='phone'" :style="{'fill':icon[type].color}" viewBox="0 0 1024 1024" :width="icon[type].size" :height="icon[type].size">
            <path :d="icon[type].icon"></path>
          </svg>
        </span>
        <span class="ml10">{{message}}</span>
      </div>
    </div>
    <div v-else class="fixed al0 at0 flex ai-c jc-c zi-9990 ar0 ab0 ba-3">
      <div :class="{'wb-70':isPhone,'wa-420 mb100':!isPhone}" class="bc-fff wi-300 sha-card ra-4">
        <div class="flex ai-c pl15 pr10 pt10 jc-b">
          ...
          <svg @click="btnSave('close')" t="1624264156037" style="fill:#888" class="icon hand" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5296" width="18" height="18">
            <path
        ...
        <div class="w-all pt10 pl15 pr15 pb10 fs-14 fc-666 lh-24">
          <input :class="{'mb5':!popValid}" v-model="popValue" v-if="type=='pop'" class="w-all fs-14 pp10 b-d h-35 mt5 ra-4" type="text">
          <p v-else>{{message}}</p>
          <span class="red fs-12" v-if="popValid">*{{popMsg}}</span>
        </div>
        <div class="w-all pl15 pr12 pb10 flex ai-c jc-e">
          <div @click="btnSave('cancel')" class="btn-default btn-min ra-4">取消</div>
          <div @click="btnSave(type=='pop'?'pop':'ok')" class="btn-primary btn-min ml10 ra-4">确定</div>
        </div>
      </div>
    </div>
  </div>
...
export default class App extends Vue {
  icon = {
    error: { size: 18, color: '#f56c6c', icon: 'M512 0C229.23 0 0 229.23 0 512s229.23 512 512 512 512-229.23 512-512S794.77 0 512 0z m169.71 568.57a80 80 0 0 1-113.14 113.14L512 625.14l-56.57 56.57a80 80 0 1 1-113.14-113.14L398.86 512l-56.57-56.57a80 80 0 1 1 113.14-113.14L512 398.86l56.57-56.57a80 80 0 1 1 113.14 113.14L625.14 512z' },
    success: { size: 18, color: '#52C41A', icon: 'M824.32 217.6c-194.56 124.928-335.872 281.6-399.36 360.96L269.312 451.072 200.704 508.928l268.288 285.184c46.592-123.392 193.024-365.056 371.712-536.576l-16.384-39.936zM512 1024C229.376 1024 0 794.624 0 512S229.376 0 512 0s512 229.376 512 512-229.376 512-512 512z' },
    warning: { size: 18, color: '#ff8a00', icon: 'M512 0C229.205333 0 0 229.205333 0 512s229.205333 512 512 512 512-229.205333 512-512S794.794667 0 512 0z m0 796.458667A56.917333 56.917333 0 1 1 511.957333 682.666667 56.917333 56.917333 0 0 1 512 796.458667z m54.186667-227.797334h0.128a60.501333 60.501333 0 0 1-53.802667 55.893334c2.048 0.256 3.882667 1.152 5.973333 1.152h-11.818666c2.048 0 3.84-0.981333 5.845333-1.109334a59.093333 59.093333 0 0 1-53.162667-55.893333l-13.056-284.16a54.314667 54.314667 0 0 1 54.613334-57.045333h26.282666a52.992 52.992 0 0 1 54.186667 57.002666l-15.146667 284.16z' }
  }
  @Prop({ type: String, default: 'success' }) type;
  @Prop({ type: String, default: '' }) message;
  @Prop({ type: String, default: '提示' }) title;
  @Prop({ type: String, default: '' }) popMsg;
  @Prop({ type: Number, default: 3000 }) time;
  @Prop({ type: Boolean, default: false }) isAuto;
  @Prop({ type: Function, default: () => { return 1 } }) close;
  @Prop({ type: Function, default: () => { return 1 } }) onClose;

  @Prop({ type: Boolean, default: false }) isModel;
  ...
  closeMsg() {
    this.$nextTick(() => {
      this.$el.addEventListener("animationend", () => {
        this.onClose();
        this.close({ messsage: this.message })
      });
    });
  ...
  btnSave(type) {
    if (type == 'pop') {
      if (!this.popValue) {
        ...
        this.close({ type, value: this.popValue });
        this.onClose();
      }
    } else {
      this.close(type);
