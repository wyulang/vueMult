<template>
  <div :class="class" class="flex-line __inputs zi-100 rel ai-c">
    <svg v-if="value&&!right" @click="inputClose" class="abs zi-120 hand input-svg abst ar6" viewBox="0 0 1024 1024" style="fill:#aaa" width="16" height="16">
    ...
    <span v-if="right" class="abs abst zi-120 fc-aaa ar6">{{right}}</span>
    <input ref="inputs" :placeholder="!right?placeholder:''" :class="{'ipt-small':size=='small','ipt-big':size=='big'}" class="ipt w-all ra-4 rel zi-110" @input="handSelect" v-model="value" :maxlength="max" :disabled="disabled" :type="type">
  </div>
</template>
